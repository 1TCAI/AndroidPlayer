...
DerryPlayer::DerryPlayer(const char *data_source, JNICallbakcHelper *helper) {
    ...
    this->data_source = new char[strlen(data_source) + 1];  //data_source_ 深拷贝
...
void *task_prepare(void *args) { // 此函数和DerryPlayer这个对象没有关系，你没法拿DerryPlayer的私有成员
    ...
    player->prepare_(); 
 //子线程函数又通过this对象去执行this的 prepare_函数
...
void DerryPlayer::prepare() {
    ...
    //子线程 task_prepare函数是无法拿到类的成员属性，所以把this对象传给子线程
    //子线程函数又通过this对象去执行this的 prepare_函数
}
...
void DerryPlayer::start() {  //这里进来的还是主线程 音视频各自需要开启一个子线程
    isPlaying = 1;
